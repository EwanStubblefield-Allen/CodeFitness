    ...
    <div class="text-center bg-neutral-dark text-light p-3">
      ...
      <div v-if="activeRoutine?.activities[0]">
        <div v-for="act in activeRoutine.activities" :key="act.id"
          class="col-12 col-md-3 card text-center m-5 p-4 fw-bold elevation-5 selectable ">
          <h2>{{ act.name }}</h2>
          ...
          <div v-if="act.equipment" class="text-start pt-4">
            ...
            <div class="text-end">
              <button class="fs-6 bg-danger border no-border rounded" @click="deleteActivity(act.id)"><span
